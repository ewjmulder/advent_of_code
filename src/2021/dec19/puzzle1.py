...
scanner_beacons_sections = Parser.from_file(SAMPLE).to_sections()
...
for scanner in scanners:
    ...
    for beacon in scanner.beacons:
        for i in range(len(path) - 1):
            step_from = path[i+1]
            step_to = path[i]
            print("step:", step_from, step_to)
            (match, scanner_position) = matches[(step_from, step_to)]
            f = scanner_position
            t = match.scanner_to_transformation.apply(beacon)
            beacon = (f[0] + t[0], f[1] + t[1], f[2] + t[2])
