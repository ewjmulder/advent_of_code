...
for i, instruction in enumerate(instructions):
    print(f"At instruction {i} of {total}")
    ...
    for cube in cubes:
        ...
        for sub_cube in sub_cubes:
            ...
            if intersection_x_from <= intersection_x_to and intersection_y_from <= intersection_y_to and intersection_z_from <= intersection_z_to:
                new_cubes.add(((intersection_x_from, intersection_y_from, intersection_z_from), (intersection_x_to, intersection_y_to, intersection_z_to)))

    if new_on_off:
        new_cubes.add((new_from, new_to))
    cubes = new_cubes


def ons(c):
    (fx, fy, fz), (tx, ty, tz) = c
    on = (tx + 1 - fx) * (ty + 1 - fy) * (tz + 1 - fz)
    return on


# print("cubes:", cubes)
all_ons = sum(ons(cube) for cube in cubes)
print(all_ons)
